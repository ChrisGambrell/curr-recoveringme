...
to authenticated
using (auth.uid() = author_id or is_conversation_member(conversation_id, auth.uid()));

create policy "Authed can insert their rows."
on public.messages for insert
to authenticated
with check (auth.uid() = author_id and is_conversation_member(conversation_id, auth.uid()));
